...
<div class="card" >
  ...
  <div class="card-body row" style="height:60vh;">
    <!-- 左边侧栏-联系人 -->
    <div class="col-3" style="max-height:100%;overflow:auto">
      <ul class="list-group list-group-flush l-name-list">
        <router-link
          v-for="(message,index) in messages.items" 
          :to="{name:'MessagesHistory',params:{id:message.recipient.id}}"
          :key="index"
        >
          <li
            class="list-group-item d-block text-truncate ">{{message.recipient.username}}</li> 
        </router-link>
    ...
    _getUserMessagesRecipients(id){
      ...
      this.$axios.get(path).then(res=>{
        ...
        let result = res.data.items.some((item,idx)=>{
          if (item.recipient && item.recipient.id==msg_who_id){
            console.log('fdafds')
        ...
        if(msg_who_id && !result){
          ...
          res.data.items.unshift(t)
        }

        this.messages = res.data
        
        this.$router.replace({name:'MessagesHistory',params:{id:this.messages.items[0].recipient.id}})
      }).catch(e=>{
        console.log(e)
      })
